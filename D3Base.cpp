...
instance access. One option is to store a pointer to D3Base instance in HWND using SetWindowLong, then retrieve it from a static method.
...
{
	return D3Base::GetApp()->MsgProc(hwnd, msg, wParam, lParam);
}



D3Base* D3Base::mApp = nullptr;

D3Base* D3Base::GetApp()
{
	return mApp;
}

float D3Base::AspectRatio() const
{
	return static_cast<float>(mClientWidth) / static_cast<float>(mClientHeight);
}

HWND D3Base::MainWnd() const
{
	return mMainWnd;
}

HINSTANCE D3Base::GetAppInst() const
{
	return mAppInst;
}

bool D3Base::Get4xMsaaState() const
{
	return m4xMsaaEnabled;
}

void D3Base::Set4xMsaaState(bool value)
{
	{
		if (m4xMsaaEnabled != value)
		{
			m4xMsaaEnabled = value;
...
D3Base::D3Base(HINSTANCE hInstance)
	: mAppInst(hInstance)
{
...
LRESULT D3Base::MsgProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	switch (msg)
	{
...
void D3Base::OnKeyDown(WPARAM wParam, LPARAM lParam)
{
}

void D3Base::OnKeyUp(WPARAM wParam, LPARAM lParam)
{
	if (wParam == VK_ESCAPE)
	{
		PostQuitMessage(0);
		...
		Set4xMsaaState(!m4xMsaaEnabled);
}

bool D3Base::InitMainWindow()
{
	WNDCLASS wc;
...
ID3D12Resource* D3Base::CurrentBackBuffer() const
{
	return mSwapChainBuffer[mCurrBackBuffer].Get();
}

D3D12_CPU_DESCRIPTOR_HANDLE D3Base::CurrentBackBufferView() const
{
...
D3D12_CPU_DESCRIPTOR_HANDLE D3Base::DepthStencilView() const
{
	return mDsvHeap->GetCPUDescriptorHandleForHeapStart();
}

void D3Base::CalculateFrameStats()
{
